...
SELFTEST_CASES: list[tuple[dict, str, list[dict], str, list[dict]]] = [
    ...
    ({"kind": "tool_args_regex", "tool": "query_mongodb",
      "pattern": r'"filename":\s*"[a-z0-9]+(_[a-z0-9]+)*"', "must": True},
     "", [{"name": "query_mongodb",
    ...
    ({"kind": "tool_pipeline_single_key", "tool": "query_mongodb"},
     "", [{"name": "query_mongodb",
           "arguments": '{"collection": "x", "pipeline": [{"$match": {"a": 1}}, {"$limit": 20}]}'}],
     "", [{"name": "query_mongodb",
           "arguments": '{"collection": "x", "pipeline": [{"$match": {"a": 1}, "fields": {}}]}'}]),
]
...
def build_arms(rule: Rule, full_prompt: str, tools: list[dict],
               skills: dict[str, str]) -> Arms:
    ...
    if rule.source == "prompt":
        return Arms(full_prompt, placebo_ablate(full_prompt, rule.text),
...
async def run_grid(client: Any, rules: list[Rule], probes: dict[str, list[str]],
                   full_prompt: str, tools: list[dict], skills: dict[str, str],
                   model_a: str, model_b: str, k: int) -> list[RunResult]:
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    print("running grid…")
    results = await run_grid(client, rules, probes, full_prompt, tools, skills,
