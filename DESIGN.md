...
flowchart TD
    ...
    subgraph P2["PHASE 2 — MEASURE (read-only, callable service)"]
        S4["ablation grid on both models<br/>+ statistics"]
    end

    subgraph P3["PHASE 3 — TRANSFORM & CERTIFY (writes)"]
        S5["repair<br/>variant funnel per broken directive"]
        S6["assemble + smoke<br/>whole-config interaction check"]
        S7["certify + audit"]
        S5 -.-> S6 --> S7
    end
...
flowchart TD
    ...
    J --> X["BEHAVIORAL CONTACT<br/>run finalists on the ~20 hardest probes<br/>on the TARGET model<br/>~10 → 2–3 measured winners"]
    X --> M["MERGE<br/>judge combines winners' strengths<br/>into one final directive"]
    M --> V{"re-run FULL probe set (MEASURE):<br/>lands in cell 2?"}
...
**Prior-art note:** the research finding that "automatic prompt rewriting is not yet reliably effective" concerns *blind* rewriting. Repair exists precisely because of that failure mode — every rewrite is scored behaviorally before acceptance (GEPA-style measured optimization; the merge combines measured winners only).
...
- **Research state:** model end-of-life migration and Continual Prompt Optimization are freshly formalized; the standing finding — prompt adaptation drives most switch-time improvement, but *blind* automatic rewriting is not reliably effective — is precisely the gap the repair funnel (3a) addresses.
...
**Threat 3 — Compliance ≠ quality (the residue).** Some of what makes an agent good is not expressible as checkable rules: reasoning depth, judgment in ambiguous cases, tone that lands. A migration can pass compliance parity and still feel worse. Implicit-contract extraction (1b) claws back part of this; not all of it. *Validation:* requires an outcome reference — where an eval bank exists, run both models and quantify the cases where compliance says parity but outcome scores diverge; that gap *is* the residue, measured. Without one: small blind human-graded samples (smell test) or production telemetry (months). *Corollary:* the approach is strongest for rule-dense, tool-heavy enterprise agents (the chosen scope) and weakest for open-ended conversational/creative agents.
...
|---|---|---|
| 0 — Proof | Hand-decomposed prompt (~20 directives), mechanical checkers for the 5 most testable, ablation grid on two models | Doubles as the validity test for threat 1 only: rerun the grid (reliability — judge verdict flips against sampling noise, or use extreme-rate rules), check it finds anything (discriminative power), ablate pairs vs. singles (independence). Threats 2–3 need their own instruments (temporal backtest; outcome reference) |
| 1 — Certifier | Automated REPRESENT + MEASURE; **read-only migration report** | Sellable alone; every report ends with "want these regressions fixed?" |
